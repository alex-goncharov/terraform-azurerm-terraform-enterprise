locals {
  encryption_password = coalesce(
    var.encryption_password,
    random_pet.default_enc_password.id,
  )


  replicated_config = base64encode(templatefile(
    "${path.module}/templates/replicated/replicated.conf",
    {
      airgap           = var.airgap.enable
  ...
  replicated_ptfe_config = base64encode(templatefile(
    "${path.module}/templates/replicated/replicated-ptfe.conf",
    {
      airgap                 = var.airgap.enable
  ...
  proxy_sh = base64encode(templatefile(
    "${path.module}/templates/cloud-init/proxy.sh",
    {
      proxy_url = var.http_proxy_url
    }
  ))

  aaa_proxy = base64encode(templatefile(
    "${path.module}/templates/cloud-init/00aaa_proxy",
    {
      proxy_url = var.http_proxy_url
    }
  ))
